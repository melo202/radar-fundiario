...
/* IDEMP-01 (P1.6, 18/08/2026): chave de idempotência DETERMINÍSTICA — hash do conteúdo
   do envio. Duplo clique, retry de rede e até F5 no meio geram a MESMA chave para o
   MESMO conteúdo: o servidor devolve o cadastro já criado em vez de duplicar. Conteúdo
   diferente (o corretor editou o form) = chave nova = ação nova, como deve ser. */
function idemKey(prefix,payload){const s=JSON.stringify(payload);let h1=5381,h2=52711;for(let i=0;i<s.length;i++){const c=s.charCodeAt(i);h1=(h1*33)^c;h2=(h2*31)^c;}return `${prefix}-${(h1>>>0).toString(36)}${(h2>>>0).toString(36)}-${s.length.toString(36)}`;}
