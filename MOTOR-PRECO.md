...
| 4 Motor estatístico | ✅ **v1 em 15/07/2026** — `estatistica.js` (puro, testado: quantis, Tukey, dedup leve entre portais, peso multiplicativo área×tipologia×qualidade×recência, confiança por regras §13) + `avaliacao.js` (filtros §6, outliers marcados com razão §9, amostra <5 = informada insuficiente §12, tudo versionado). **1º ValuationResult real:** apto 90 m²/3q Setor Bueno → R$ 891.508 (R$ 9.906/m² ponderado; mediana 9.594), faixa R$ 621 mil–1,14 mi, confiança "baixa" com fatores declarados (9 comparáveis, dispersão 60%, oferta≠transação), 9 comparáveis rastreáveis com peso cada. Endpoint `POST /motor/avaliar` + `GET /motor/avaliacoes/:id`. Pendências da fase: dispersão alta pede segmentação por padrão construtivo e mais amostra (varredura acumulando); MAD como segunda cerca. | Fase 3 |
