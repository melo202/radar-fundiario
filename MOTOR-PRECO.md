...
| 3 Comparáveis | **Ingestão fonte A NO AR (15/07/2026):** Brave Search (conteúdo público indexado, sem scraping de portal) → extração pela cadeia de IA → `listings`/`properties`/`price_history`/`audit_log`, dedup de 1º nível por (portal,url,hash), endpoints protegidos por MOTOR_TOKEN. 1ª colheita real: 18/18 extraídos sem falha — e revelou o desafio previsto no §6: a busca devolve muitas PÁGINAS-CATÁLOGO dos portais (sem preço/área individuais; ex.: extrator leu "3.703 imóveis" como área). **Próximo incremento:** score de qualidade + filtros mínimos (§ quality/§6 — imóvel sem preço confiável não vira comparável), viés de consulta para snippets com R$, regra "área exige m² explícito" no prompt, e avaliar coleta da página individual nas fontes que permitem. B (manual) e C (parcerias) entram pela mesma tabela. Depois: dedup multi-sinal (§5), geocoding CNEFE (§7), pontuação (§6). | Fase 2 ✅ |
