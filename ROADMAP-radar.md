...
- 🔶 **R2 — Marca no produto inteiro (iniciado 15/07 após aprovação do protótipo).** FEITO: login do painel virou Cidade Viva de verdade (canvas + limite servido pelo motor + paleta do kit no painel inteiro) e o splash do app abre com a logo oficial (fallback textual se a imagem falhar). FALTA: favicon/ícones PWA/manifest, cabeçalho do app, laudo, index de fallback. Plano original: Logo no cabeçalho e no splash de boot, favicon + ícones PWA (192/512/apple-touch) gerados do kit, `manifest.json`, título das páginas, `index.html` de fallback, tela de login do painel, cabeçalho do Relatório de Referência/Ficha (o laudo é a peça que o cliente do corretor vê — capricho dobrado).
