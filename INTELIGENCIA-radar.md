...
2. **Em compensação, o servidor aceita** (tudo testado, JSONP-compatível): `returnCountOnly` (~200 bytes/resposta), `orderByFields`, `returnDistinctValues`, `outFields` específico (verificado em produção 2026-07-09/10 — a afirmação antiga de "só com returnDistinctValues" estava errada para o cadastro; quem exige `outFields=*` é o serviço `Mapa_ModeloEspacial`/Plano Diretor), **aritmética no WHERE** (`vlvenal/areaedif < 3000` funciona!) e **busca espacial por raio em metros** (`distance=300&units=esriSRUnit_Meter`). Isso permite mediana/quartis **exatos** de R$/m² de qualquer recorte por busca binária de contagens — sem baixar registro nenhum.
